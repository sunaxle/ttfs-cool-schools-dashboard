        ...
        // Background click clears active golden paths and resets UI
        /**
         * Clears the active highlighted paths and dismisses the matrix UI overlay,
         * restoring the D3 force simulation to its resting state.
         */
        function closeMatrix() {
            document.getElementById('impact-matrix-overlay').classList.remove('active');
            node.classed("dimmed", false).classed("highlighted", false);
            link.classed("dimmed", false).classed("highlighted", false);
            simulation.alpha(0.3).restart();
        }

        document.getElementById('btnCloseMatrix').addEventListener('click', closeMatrix);
        ...
        const node = group.append("g")
            ...
            .on("click", function(event, d) {
                ...
                // ----------------------------------------------------
                // BUILD IMPACT MATRIX DYNAMIC UI OVERLAY
                // ----------------------------------------------------
                const leafName = d.data.name;
                document.getElementById('matrix-leaf-name').innerText = leafName.replace("Student ", "Student: ").replace("Researcher: ", "Researcher: ");

                const tableBody = document.getElementById('matrix-body');
                tableBody.innerHTML = ""; // Clear previous data

                // Scrape exactly which active nodes lit up
                const activeNetwork = nodes.filter(n => highlightedNodes.has(n.data.id));

                // Sort them ascending from deepest structural roots to the upper canopy
                activeNetwork.sort((a,b) => a.data.group - b.data.group);

                activeNetwork.forEach(n => {
                    // Ignore empty structural joints or identical self
                    if(n.data.isFiller || n.data.id === d.data.id) return;

                    let categoryStr = "Network Node";

                    // Categorize computationally
                    if (n.data.group === 0) {
                        categoryStr = "Primary Hub";
                    } else if (n.data.group > 0) {
                        categoryStr = "Academic Lineage";
                    } else if (n.data.group < 0) {
                        const searchName = n.data.name.toLowerCase();
                        if(searchName.includes("funder") || searchName.includes("usda") || searchName.includes("nifa") || searchName.includes("grant")) {
                            categoryStr = "Funder";
                        } else if (searchName.includes("farmer") || searchName.includes("market") || searchName.includes("organic") || searchName.includes("cover crop")) {
                            categoryStr = "Agricultural Partner";
                        } else if (searchName.includes("science") || searchName.includes("ecology") || searchName.includes("biology") || searchName.includes("math")) {
                            categoryStr = "Science Domain";
                        } else if (searchName.includes("network") || searchName.includes("distrib")) {
                            categoryStr = "Structural Channel";
                        } else {
                            categoryStr = "Community Node";
                        }
                    }

                    const row = document.createElement("tr");
                    row.innerHTML = `
                        <td class="td-category">${categoryStr}</td>
                ...
                // Ignite UI
                document.getElementById('impact-matrix-overlay').classList.add('active');
            });
        ...
        /**
         * Initializes drag behavior for the nodes, allowing user interaction to pull
         * nodes around the force-directed graph. The trunk (group 0) is explicitly pinned.
