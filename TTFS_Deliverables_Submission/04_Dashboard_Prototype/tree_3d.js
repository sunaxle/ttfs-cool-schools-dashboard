document.addEventListener('DOMContentLoaded', () => {
    ...
    for (let y = 2025; y <= 2060; y++) {
        ...
        baseHeatData.push(parseFloat((102 + (yPct * 4)).toFixed(1)));
        mitigatedHeatData.push(parseFloat((98 - (yPct * 8)).toFixed(1)));
    }

    const baseGradient = ctx.createLinearGradient(0, 0, 0, 300);
    baseGradient.addColorStop(0, 'rgba(216, 67, 21, 0.25)');
    baseGradient.addColorStop(1, 'rgba(216, 67, 21, 0.02)');

    const shadeGradient = ctx.createLinearGradient(0, 0, 0, 300);
    shadeGradient.addColorStop(0, 'rgba(46, 125, 50, 0.35)');
    shadeGradient.addColorStop(1, 'rgba(46, 125, 50, 0.05)');
    ...
    const tempChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                ...
                {
                    label: 'Mitigated Microforest Shade Temp (°F)',
                    borderColor: '#2e7d32',
                    borderWidth: 3,
                    backgroundColor: shadeGradient,
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    backgroundColor: 'rgba(27, 77, 43, 0.95)',
    ...
    function updateChartTracer(yearVal) {
        const index = Math.max(0, Math.min(35, yearVal - 2025));
        const baseTemp = baseHeatData[index];
        const shadeTemp = mitigatedHeatData[index];
        const diff = (shadeTemp - baseTemp).toFixed(1);

        const lblBaseTemp = document.getElementById("lblBaseTemp");
        const lblShadeTemp = document.getElementById("lblShadeTemp");
        const lblCoolingDiff = document.getElementById("lblCoolingDiff");

        if (lblBaseTemp) lblBaseTemp.textContent = `${baseTemp.toFixed(1)}°F`;
        if (lblShadeTemp) lblShadeTemp.textContent = `${shadeTemp.toFixed(1)}°F`;
        if (lblCoolingDiff) lblCoolingDiff.textContent = `${diff}°F`;
    ...
    // ==========================================
    // CHART 2: Canopy Surface Area & Air Volume Growth
    // ==========================================
    const canvas2 = document.getElementById('canopyGrowthChart');
    let canopyGrowthChart = null;
    const canopyAreaArray = [];
    const airVolumeArray = [];
    ...
    if (canvas2) {
        ...
        for (let y = 2025; y <= 2060; y++) {
            const yPct = (y - 2025) / 35;
            // Canopy Area (sq ft): Starts at 18.5k sq ft in 2025 and expands to 148.5k sq ft in 2060
            const area = Math.round(18500 + (Math.pow(yPct, 1.25) * 130000));
            canopyAreaArray.push(area);

            // Cooled Air Shield Volume (cu ft): Area * Effective Cooling Height Dome
            const vol = Math.round(area * (15 + (yPct * 10)));
            airVolumeArray.push(vol);
        }

        const areaGradient = ctx2.createLinearGradient(0, 0, 0, 300);
        areaGradient.addColorStop(0, 'rgba(46, 125, 50, 0.45)');
        areaGradient.addColorStop(1, 'rgba(46, 125, 50, 0.05)');

        canopyGrowthChart = new Chart(ctx2, {
            type: 'line',
            data: {
                labels: labels,
                datasets: [
                    {
                        label: 'Cumulative Canopy Surface Area (sq ft)',
                        borderColor: '#2e7d32',
                        borderWidth: 3,
                        backgroundColor: areaGradient,
                        data: canopyAreaArray,
                        fill: true,
                        tension: 0.35,
                        yAxisID: 'yArea',
                        pointRadius: 2,
                    ...
                    {
                        label: 'Cooled Microclimate Air Volume (cu ft)',
                        borderColor: '#0288d1',
                        borderWidth: 3,
                        borderDash: [5, 5],
            ...
            options: {
                        ...
                        callbacks: {
                            label: function(context) {
                                if (context.datasetIndex === 0) {
                                    ...
                                    return ` ${context.dataset.label}: ${context.raw.toLocaleString()} cu ft`;
                                }
                            }
                        }
                    }
                ...
                scales: {
                    ...
                    yArea: {
                        ...
                        title: {
                            display: true,
                            text: 'Canopy Surface Area (sq ft)',
                            font: { size: 13, weight: 'bold' },
                            color: '#2e7d32'
                        },
                        ticks: {
                            font: { size: 12 },
                            callback: function(value) { return (value / 1000).toFixed(0) + 'k sq ft'; }
                        }
                    },
                    yVol: {
                        type: 'linear',
                        ...
                        title: {
                            display: true,
                            text: 'Cooled Air Volume (cu ft)',
                            font: { size: 13, weight: 'bold' },
                            color: '#0288d1'
                        },
                        ticks: {
                            font: { size: 12 },
                            callback: function(value) { return (value / 1000).toFixed(0) + 'k cu ft'; }
                        }
                    }
                }
            }
    ...
    function updateCanopyGrowthTracer(yearVal) {
        if (!canopyGrowthChart) return;
        const index = Math.max(0, Math.min(35, yearVal - 2025));
        const area = canopyAreaArray[index];
        const vol = airVolumeArray[index];
        const yPct = index / 35;
        const pockets = Math.round(4 + (yPct * 18));

        const lblCanopyArea = document.getElementById("lblCanopyArea");
        const lblAirVolume = document.getElementById("lblAirVolume");
        const lblThermalPockets = document.getElementById("lblThermalPockets");

        if (lblCanopyArea) lblCanopyArea.textContent = `${area.toLocaleString()} sq ft`;
        if (lblAirVolume) lblAirVolume.textContent = `${vol.toLocaleString()} cu ft`;
        if (lblThermalPockets) lblThermalPockets.textContent = `${pockets} Active Zones`;

        canopyGrowthChart.setActiveElements([
            { datasetIndex: 0, index: index },
            { datasetIndex: 1, index: index }
        ]);
        canopyGrowthChart.update('none');
    }

    /**
     * Reads the current value of the timeline year slider, updates the 3D Temporal Projection scene
     * (tree growth and shadow changes), and advances the Chart.js visual tracers.
     */
